...
const renderComments = () => {
  const currentComents = comments.slice(showingComments, showingComments + COMMENT_COUNTER);
  showingComments += COMMENT_COUNTER;
  showingComments = Math.min(showingComments, comments.length);
  currentComents.forEach((comment) => socialCommentsList.append(createComment(comment)));
  fillCommentCount();
